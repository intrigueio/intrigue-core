      ...
      if entity.type_string == "FtpServer"
        ...
        unless (entity.created_by?("uri_brute") || entity.created_by?("uri_spider") )
          ...
          start_recursive_task(task_result,"uri_brute", entity, [
            ...
            {"name" => "user_list", "value" => "admin,test,server-status,.svn,.git,wp-config.php,
              config.php,configuration.php,LocalSettings.php,mediawiki/LocalSettings.php,mt-config.cgi,
              mt-static/mt-config.cgi,settings.php,.htaccess,config.bak,config.php.bak,config.php~,
              #config.php#,config.php.save,.config.php.swp,config.php.swp,config.php.old"}])
        end
      else
