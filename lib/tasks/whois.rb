...
class WhoisTask < BaseTask
  ...
  def run
    ...
    begin
      ...
      @task_result.log "Unable to query whois: #{e}"
    rescue Whois::ResponseIsThrottled => e
      @task_result.log "Got a response throttled message: #{e}"
      sleep 10
      return run # retry
    rescue StandardError => e
      @task_result.log "Unable to query whois: #{e}"
    rescue Exception => e
      @task_result.log "UNKNOWN EXCEPTION! Unable to query whois: #{e}"
    end
    ...
    if answer
      ...
      @task_result.log "== Full Text: =="
      @task_result.log answer.parts.first.body
      @task_result.log "================"
      ...
      if @entity.type_string == "DnsRecord"
            ...
            if nameserver.to_s =~ /\d\.\d\.\d\.\d/
              ...
              begin
                ...
                _create_entity "DnsServer", "name" => ip_address
                  ...
                  @task_result.log "Unable to look up host: #{e}"
              end
            end
          end
        end
        ...
        begin
          if answer.technical_contact
            @task_result.log "Creating user from technical contact"
            _create_entity("Person", {"name" => answer.technical_contact.name})
          end
        rescue Exception => e
          @task_result.log "Unable to grab technical contact"
        end
        ...
        begin
          if answer.admin_contact
            @task_result.log "Creating user from admin contact"
            _create_entity("Person", {"name" => answer.admin_contact.name})
          end
        rescue Exception => e
          @task_result.log "Unable to grab admin contact"
        end
        ...
        begin
          if answer.registrant_contact
            @task_result.log "Creating user from registrant contact"
            _create_entity("Person", {:name => answer.registrant_contact.name})
          end
        rescue Exception => e
          @task_result.log "Unable to grab registrant contact"
        end
      ...
      @task_result.log "Domain WHOIS failed, we don't know what nameserver to query."
    end
