...
class BaseTask
  ...
  def perform(task_id, handlers, hook_uri=nil)
    ...
    #
    @task_result.log "Id: #{task_id}"
    @task_result.log "Entity: #{@entity.type_string}##{@entity.name}"
    @task_result.log "Options: #{options}"
    ...
    unless allowed_types.include?(@entity.type_string) || allowed_types.include?("*")
      ...
      @task_result.log_error "Unable to call #{self.metadata[:name]} on entity: #{@entity}"
      broken_input = true
    end
      ...
      if setup(task_id, @entity, options)
        begin
          Timeout.timeout($intrigue_global_timeout) do # 15 minutes should be enough time to hit a class b for a single port w/ masscan
            @task_result.log "Calling run()"
            # Save the task locally
            @task_result.save
            # Run the task, which will update @task_result and @task_result
            run()
            @task_result.log_good "Run complete. Ship it!"
          end
        rescue Timeout::Error
          @task_result.log_error "ERROR! Timed out"
        end
      else
        @task_result.log_error "Setup failed, bailing out!"
      end
    end
    ...
    handlers.each do |handler_type|
      @task_result.log "Processing #{handler_type} handler!"
      begin
        ...
        @task_result.log_error "Unable to process handler #{handler_type}: #{e}\N"
        @task_result.log_error "Got response: #{response}"
      end
    end
    ...
    @task_result.log_good "Marking task complete!"
    @task_result.timestamp_end = Time.now.getutc
    @task_result.complete = true
    @task_result.log "Calling cleanup!"
    @task_result.save
  end
  ...
  def setup(task_id, entity, user_options)
    ...
    if user_options
      ...
      @task_result.log "Got user options list: #{user_options}"

      # for each of the user-supplied options
      user_options.each do |user_option| # should be an array of hashes

        #@task_result.log "Processing user option: #{user_option}"

        # go through the allowed options
        allowed_options.each do |allowed_option|

          #@task_result.log "Checking against allowed option list: #{allowed_option}"

          # if we have a match
          if "#{user_option["name"]}" == "#{allowed_option[:name]}"

            #@task_result.log "Verifying this user option: #{user_option["name"]}"

            ###
            ### Match the user option against it's specified regex
            ###

            #@task_result.log "Allowed option: #{allowed_option}"

            # XXX - we need to regex the option in order to accept it
            if allowed_option[:regex] == "integer"
              #@task_result.log "Regex should match an integer"
              regex = /^-?\d+$/
            elsif allowed_option[:regex] == "boolean"
              #@task_result.log "Regex should match a boolean"
              regex = /(true|false)/
            elsif allowed_option[:regex] == "alpha_numeric"
              #@task_result.log "Regex should match an alpha-numeric string"
              regex = /^[a-zA-Z0-9\_\;\(\)\,\?\.\-\_\/\~\=\ \,\?\*]*$/
            elsif allowed_option[:regex] == "alpha_numeric_list"
              #@task_result.log "Regex should match an alpha-numeric list"
              regex = /^[a-zA-Z0-9\_\;\(\)\,\?\.\-\_\/\~\=\ \,\?\*]*$/
            elsif allowed_option[:regex] == "filename"
              #@task_result.log "Regex should match a filename"
              regex = /(?:\..*(?!\/))+/
            elsif allowed_option[:regex] == "ip_address"
              #@task_result.log "Regex should match an IP Address"
              ...
              @task_result.log_error "Unspecified regex for this option #{allowed_option[:name]}"
              @task_result.log_error "FATAL! Unable to continue!"
              return nil
            end

            # Run the regex
            unless regex.match "#{user_option["value"]}"
              @task_result.log_error "Regex didn't match"
              @task_result.log_error "Option #{user_option["name"]} does not match regex: #{regex.to_s} (#{user_option["value"]})!"
              @task_result.log_error "FATAL! No task processing since regex didn't match option!"
              return nil
            end
            ...
            if allowed_option[:type] == "Integer"
              # convert to integer
              #@task_result.log "Converting #{user_option["name"]} to an integer"
              user_option["value"] = user_option["value"].to_i
            elsif allowed_option[:type] == "String"
              # do nothing, we can just pass strings through
              #@task_result.log "No need to convert #{user_option["name"]} to a string"
              ...
              #@task_result.log "Converting #{user_option["name"]} to a bool"
              ...
              @task_result.log_error "FATAL! Don't know how to handle this option when it's given to us as a string."
              return nil
            end

            # hurray, we can accept this value
            #@task_result.log_good "Congrats! Allowed this user option: #{user_option}"
            @user_options << { allowed_option[:name] => user_option["value"] }
          end
        end

      end
      @task_result.log "Task configured with the following options: #{@user_options}"
    else
      @task_result.log "No User options"
    end
  ...
  private
    ...
    def _unsafe_system(command)
      ...
      if command =~ /(\||\;|\`)/
        ...
        @task_result.log_error "FATAL Illegal character in #{command}"
        return
      end
    ...
    def _create_entity(type, hash)
      @task_result.log_good "Creating entity: #{type}, #{hash.inspect}"
      ...
      #  @task_result.log_error "Unable to create entity: #{type}, #{hash.inspect}"
      #  @task_result.log_error "ERROR: #{e}"
      #end

      # If we don't get anything back, safe to assume we can't move on
      unless entity
        @task_result.log_error "SKIPPING Unable to verify & save entity: #{type} #{hash}"
        return
      end
    ...
    def _get_option(name)
      ...
      #@task_result.log "Option configured: #{name}"
