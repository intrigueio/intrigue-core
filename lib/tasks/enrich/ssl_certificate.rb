    ...
    module Enrich
      class SslCertificate < Intrigue::Task::BaseTask
        ...
        def parse_time(time_string)
          ...
          DateTime.strptime(time_string, '%Q')
        end
        ...
        def run
          ...
          # not before aug 31
          # today aug 20
          if not_before && parse_time(not_before) > Time.now
            _log 'Creating issue for certificate that is not yet valid'
            _create_linked_issue 'invalid_certificate_premature',
                                 { proof: "Certificate is valid starting from #{not_before}, tested on #{now}" }
          end

          not_after = _get_entity_detail('not_after')
          # not after aug 31
          # today aug 20
          if not_after && parse_time(not_after) < Time.now
          ...
          almost_expired_time = if _get_entity_detail('issuer') =~ /Let's Encrypt/
                                  604_800 # seven days
                                else
          ...
          if not_after &&
             !(parse_time(not_after) < Time.now) && # expired
             (parse_time(not_after) - almost_expired_time) < Time.now # expires in 30 days
