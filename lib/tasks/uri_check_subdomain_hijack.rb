...
class UriCheckSudomainHijack  < BaseTask
  ...
  def run
    ...
    if response_body
      ...
      elsif response_body.match /The gods are wise, but do not know of the site which you seek\!/i
        _create_hijackable_subdomain_issue "Pantheon", uri, "potential" unless (uri =~ /pantheon.io/ || uri =~ /amazonaws.com/)

      elsif response_body.match(/Public Report Not Activated/) || response_body.match(/This public report page has not been activated by the user/)
      ...
      #elsif response_body.match /page not found/i
