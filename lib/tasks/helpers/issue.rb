...
module Issue
  ...
  def _exposed_server_identified(regex, name=nil, type="Development")
    ...
    _create_issue({
      name: "#{type} System Identified: #{exposed_name}",
  ...
  def _create_weak_service_issue(ip_address, port, proto, tcp)
    ...
    _create_issue({
      name: "Weak Service Identified: #{proto} on #{port}/#{transport}",
  ...
  def _create_missing_cookie_attribute_http_only_issue(uri, cookie, severity=5)
    _create_issue({
      name: "Insecure cookie detected on #{uri}: missing 'httpOnly' attribute",
      type: "insecure_cookie_detected",
      severity: severity,
      status: "confirmed",
      description: "A cookie was identified without the 'httpOnly' cookie attribute",
      references: [],
      details: { 
        uri: uri,
        cookie: cookie 
  ...
  def _create_missing_cookie_attribute_secure_issue(uri, cookie, severity=5)
    _create_issue({
      name: "Insecure cookie detected on #{uri}: missing 'secure' attribute",
      type: "insecure_cookie_detected",
      severity: severity,
      status: "confirmed",
      description: "A cookie was identified without the 'secure' cookie attribute",
      references: [],
      details: { 
        uri: uri,
        cookie: cookie 
