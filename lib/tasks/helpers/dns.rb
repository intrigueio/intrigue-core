    ...
    module Dns
      ...
      def create_dns_entity_from_string(s, alias_entity = nil, unscoped = false, more_deets = {})
        ...
        if s.is_ip_address?
          ...
          entity_details['name'] = s.to_s.strip.gsub(/^\*\./, '').gsub(/\.$/, '')
          e = if parse_domain_name(entity_details['name']) == entity_details['name']
      ...
      def gather_wildcard_resolutions(suffix, create_exhaustive_list = false)
        ...
        10.times do
          random_string = "#{(0...8).map { rand(65..90).chr }.join.downcase}.#{suffix}"
          ...
          all_discovered_wildcards.concat resolved_addresses unless resolved_addresses.empty?
        end
        ...
        if all_discovered_wildcards.count > 1 && create_exhaustive_list
          _log "Multiple wildcard ips for #{suffix} after resolving these: #{all_discovered_wildcards.map do |x|
                                                                               x['name']
                                                                             end }."
          ...
          until no_new_wildcards || attempts > max_attempts
            ...
            (all_discovered_wildcards.count * 20).times do |_x|
              random_string = "#{(0...8).map { rand(65..90).chr }.join.downcase}.#{suffix}"
              resolved_addresses = resolve(random_string)

              # keep track of it unless we already have it
              newly_discovered_wildcards.concat resolved_addresses unless resolved_addresses.empty?
            end
        ...
        # Then everything else
          ...
          begin
            ...
            out = resources.map do |r|
              ...
              record_data = r.address.to_s if %w[A AAAA].include?(record_type) # && r.respond_to?("name")
              ...
              record_data = { 'value' => r.value.to_s, 'tag' => r.tag.to_s } if record_type == 'CAA'
      ...
      def collect_soa_details(lookup_name)
        ...
        { 'primary_name_server' => (data['mname']).to_s,
          'responsible_party' => (data['rname']).to_s,
          'serial' => data['serial'] }
      end
        ...
        response.each do |r|
          r['lookup_details'].each do |record|
            ...
            mx_records << {
              ...
              'host' => (record['response_record_data']['exchange']).to_s
            }
          end
        end
      ...
      def collect_resolutions(results)
        ...
        dns_entries.uniq.each do |o|
          _create_linked_issue('resolves_to_localhost', { details: o }) if o['response_data'] == '127.0.0.1'
        end
