...
class DnsBruteTldTask < BaseTask
  ...
  def run
    ...
    if opt_use_file
      ...
      @task_result.log "Using built-in list"
      tld_list = ['biz', 'cc', 'co.uk', 'com', 'cn', 'de', 'edu', 'info',
        'int', 'io', 'net', 'org', 'mobi', 'name', 'pro', 'xxx', 'us']
    end

  if opt_cctld
    @task_result.log "Country code TLDs configured"
...
=end
    ...
    @task_result.log_good "Using tld list: #{tld_list}"
    ...
    tld_list.each do |tld|
      begin
        ...
        @task_result.log_good "Resolved Address #{resolved_address} for #{domain}" if resolved_address
