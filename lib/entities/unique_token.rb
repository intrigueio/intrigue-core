...
class UniqueToken < Intrigue::Core::Model::Entity
  ...
  def self.supported_token_types
    [
      { provider:"google_analytics", regex: /^UA-[\d\-]+$/i, matcher: /(UA-[\d\-]+)/i, sensitive: false },
      { provider:"google_adsense", regex: /^pub-\d+$/, matcher: /(pub-\d+)/, sensitive: false },
      { provider:"hotjar", regex: /^[\d+]$/, matcher: /_hjSettings=\{hjid:([\d+]),/i, sensitive: false },
      { provider:"intercom", regex: /^[\w\d]{0,8}$/, matcher: /app_id: \"([\w\d]{0,8})\"\,/i, sensitive: false },
      { provider:"slack_person", regex: /^xoxp-[0-9A-Za-z\\-]{72}$/, matcher: /(xoxp-[0-9A-Za-z\\-]{72})/i, sensitive: true },
