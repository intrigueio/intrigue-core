...
class HandleResultWorker
  ...
  def perform(id, force_handling)
    ...
    ### So... here we wait for the result's tasks to complete... unless we hit our
    ### countdown timer, or the user has specifically requested that we don't (This
    ### is handy in cases of requesting to handle the results via CLI)

    # failsafe ... never let us wait more than sleep_length * i
    sleep_length = 60
    i = 20
    ...
    until (incomplete_task_count == 0 || i < 0 || force_handling==true)
      ...
      puts "HandleResultWorker #{id}: Countdown... #{i}"
      sleep sleep_length
