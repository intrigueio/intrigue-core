class CoreApp < Sinatra::Base
  ...
  get '/:project/entities' do
    ...
    params[:only_scoped] == "on" ? @only_scoped = true : @only_scoped = false
    ...
    @selected_entities = @selected_entities.where(:scoped => true) if @only_scoped
    ...
    if params[:export] == "csv"
      ...
      csv_result = ""
      @selected_entities.paged_each(:rows_per_fetch => 300){ |e| csv_result << "#{e.export_csv}\n" }
      return csv_result
      ...
      result = []
      @selected_entities.paged_each(:rows_per_fetch => 300){ |e| result << "#{e.export_json(true)}" }
