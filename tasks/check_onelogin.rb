...
class CheckOnelogin  < BaseTask
  ...
  def metadata
    {
      ...
      :authors => ["jcran"],
      :description => "This task checks Onelogin for the presence of an account.",
      :references => [],
  ...
  def run
    ...
    uri = "https://#{name}.onelogin.com/login"
    ...
    target_strings = [
      { :regex => /Logo_default\@2x/i,
        :entity_type => "Uri",
        :entity_name => "#{uri}",
        :entity_content => "Onelogin Account Found" }
    ]
